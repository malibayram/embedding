...
class GemmaForCausalLM(nn.Module):
  def __init__(
    ...
    self.tokenizer = tokenizer if tokenizer is not None else TurkishTokenizer()
  ...
  def load_weights_from_hf(self, model_state_dict):
     ...
     Key mappings:
     - embed_tokens.weight -> embedder.weight (with size adjustment if needed)
     ...
     for key, value in model_state_dict.items():
         if key == 'embed_tokens.weight':
             # Handle embedding layer with size mismatch
             hf_vocab_size, hidden_size = value.shape
             current_vocab_size = self.config.vocab_size
             
             print(f"HF model vocab size: {hf_vocab_size}, Current model vocab size: {current_vocab_size}")
     ...
     if missing_keys:
         print(f"Missing keys: {missing_keys}")  # Show missing keys
     if unexpected_keys:
         print(f"Unexpected keys: {unexpected_keys}")  # Show unexpected keys
  ...
  def save_pretrained(self, save_directory: str):
     ...
     # Save the complete model state dict including embedder
